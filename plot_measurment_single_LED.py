...
save = 1
dirpath='./measured spectra/Measurement/'
...
# listoffiles = [x for x in listoffiles if 'LED390' in x]
...
for file in listoffiles:
    ...
    current = float(header[np.where(header == 'Input current:')[0][0]][1])
    current = convert_current_to_mA(current)
...
if save:
    plt.savefig(dirpath+ "LED" + ".png",dpi=300)
else:
    plt.show()
