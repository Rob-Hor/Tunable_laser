...
def plot_dirpath(path_to_directory, ax=None):
    ...
    for i in range(len(np.unique(current))):
        ...
        cs = ax.pcolormesh(x,y,z, cmap=cmap, norm=LogNorm(vmin=vmin, vmax=vmax))
        ...
        teplota_end = np.genfromtxt(fname=path_to_directory+"konečná teplota.txt",dtype=float,converters={0:convertcommas_bitwise})
        teplota_start = header[np.where(header == 'Notes:')[0][0]][1].split("(K) = ", 1)[1][:6].replace(",",".")
        teplota = np.round((float(teplota_start)+ teplota_end)/2,1)
        # teplota = dirsubpath[20:-10]
        
        ax.set_title('Čerpací proud: '+str(current[current_index_low]).replace(".",",") +' mA'+
                     "\n"+"Teplota: "+str(teplota).replace(".",",") + " K")  
...
fig, axs = plt.subplots(3, 4, figsize=(20,13), sharey=True, sharex=True)
for dirsubpath in dirpaths_sorted:
    dirpath = dirpaths + dirsubpath + '/'
    
    if "44 mA" in dirsubpath: i = 0
    elif "40 mA" in dirsubpath: i = 1
    elif "37 mA" in dirsubpath: i = 2
    
    if "min" in dirsubpath: j = 0
    elif "pokojová" in dirsubpath: j = 1
    elif "mírný" in dirsubpath: j = 2
    elif "max" in dirsubpath: j = 3
    plot_dirpath(dirpath, axs[i,j])
    # plot_dirpath(dirpath)
axs[0,2].axis("off")
axs[1,2].axis("off")
...
if save:
    ...
    plt.savefig(dirpaths+ "mega mapa.png",dpi=300)
else:
    plt.show()
